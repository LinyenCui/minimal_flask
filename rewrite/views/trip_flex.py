...
# 司機圖示：用計程車 🚕 而不是轎車 🚗 —— 台灣俗稱計程車「小黃」，
# 🚕 在各家字型都是黃色車身，跟 🚗 同寬（約 2 個字寬），等於免費換到辨識色。
DRIVER_ICON = "🚕"


def driver_label(driver_id) -> str:
    """列表欄位用的司機編號文字（圖示 + 完整編號）。

    2026-09-20 用戶回報：列表裡司機編號被截成「28…」「61…」，要求
    「把司機編號全部顯示出來，但不可以動到金額顯示」。

    為什麼會截：carousel 的 bubble 是 kilo（260px），一行要塞
    編號/時間/路線/司機(/金額)，司機欄原本只分到 ~35px；而
    圖示(~13px) + 5 碼編號(~29px) = ~42px → 差 7px。
    （順帶一提：`size` 已經是 `xxs`，**沒有更小的關鍵字級**，縮字這條路不存在。）

    ⚠️ 第一版同時做了「拿掉圖示」＋「路線讓一格給司機欄」——
    **做過頭了**。光是讓一格，司機欄就有 52px，塞得下 42px 的圖示版，
    還餘 10px。所以圖示留著，只保留加寬。
    （用戶追問「看能否把車子圖示弄回來」才發現，記在這裡免得有人又拿掉。）

    編號欄的 `#` 目前不必拿掉（「#5010」29px / 配到 35px）。
    但班次編號一旦進到 5 碼（「#12345」剛好 35px，零餘裕）就得拿掉 `#` ——
    trip_id 目前 5 千多，大約一年後會遇到。
    """
    return f"{DRIVER_ICON}{driver_id}" if driver_id else f"{DRIVER_ICON}?"
