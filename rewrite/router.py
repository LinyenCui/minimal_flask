...
支援命令（自然語法，無前綴）：
  查客戶 <term>      — 模糊查詢（cascade fallback：簡稱→身分證→病歷號→姓名→地址）
  客戶詳情 <id>      — 單筆 ID 查詢
...
# 命令 regex
_RE_QUERY = re.compile(r'^查客戶(?:\s+(.+))?$')          # 查客戶 / 查客戶 X
_RE_DETAIL = re.compile(r'^客戶詳情\s+(\d+)$')             # 客戶詳情 54
_RE_LAYER = re.compile(r'^病歷層\s+(\d+)$')                # 病歷層 23
_RE_LAYER_SUMMARY = re.compile(r'^病歷層分布$')            # 病歷層分布
...
def try_route(event) -> bool:
    ...
    # 提早退出：不像我們的命令
    if not (text.startswith('查客戶') or text.startswith('客戶詳情')
            or text.startswith('病歷層')):
        return False
    ...
    try:
        ...
        if m:
            return _handle_detail(reply_token, session, int(m.group(1)))
        ...
        if m:
            ...
            if not term:
                ...
                return True
            return _handle_query(reply_token, session, term)
...
def _handle_detail(reply_token, session, customer_id: int) -> bool:
...
def _handle_query(reply_token, session, term: str) -> bool:
