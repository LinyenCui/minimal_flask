...
- trip_mutation：改現在態班次（trips 表，未執行/今天）
  例：「1077請假化療-30」「99502註銷」「指派司機533給1077」「改乘客名 1077 ...」
       「現在態班次1077改類別東洋」（key 錯時用）
- completed_trip：查/改過去態已完成班次（completed_trips 表）+ 金額統計
  查詢例：「查已完成 昨天」「4/26-5/2 東洋班次加總」「查看 820」「司機5386本月車資總和」
       「昨天診所班次」「上週司機533收入」「7月東洋總計」
  修改例：「修改 #820 金額 1250 加成 750 等候3小時」「記錄車資 820 1250 750 加班費」
       「已完成班次820改類別診所 報帳分類錯誤」
- customer：客戶資料 CRUD
...
- 過去態 vs 現在態的判定（**重要**）：
  ...
  * 「修改 #N 金額」「記錄車資 N」「#N 改類別」「修改 N」「修改 N $XXX」「過去態 N」→ completed_trip
...
「修改 #820 金額 1250 加成 750」 → completed_trip
「記錄車資 820 1250 750 等候3小時」 → completed_trip
