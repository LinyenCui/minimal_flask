...
def generate_weekly_report_sqlite(category=None):
    try:
        # 獲取日期範圍
        ...
        SELECT 
            ...
            meter_fare, 
            extra_fare,
            COALESCE(meter_fare, 0) + COALESCE(extra_fare, 0) as actual_fare,
            driver_id
        FROM 
            completed_trips
        WHERE 
        ...
        params = [last_sunday.isoformat(), last_saturday.isoformat()]
        ...
        filename = f"weekly_report_sqlite{category_suffix}_{report_date}.xlsx"
...
# 執行測試
print("開始生成週報表...")
result, filename = generate_weekly_report_sqlite()
print(result)
