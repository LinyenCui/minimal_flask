...
def truncate_and_copy(local_conn, render_conn, table_name):
    ...
    with local_conn.cursor() as local_cur, render_conn.cursor(cursor_factory=DictCursor) as render_cur:
        try:
            ...
            # 🔥 修復：避免 CASCADE 清空 completed_trips
            # 檢查是否需要保護 completed_trips
            if table_name == 'drivers':
                local_cur.execute("SELECT COUNT(*) FROM completed_trips;")
                completed_count = local_cur.fetchone()[0]
                if completed_count > 0:
                    print(f"   - 檢測到 {completed_count} 筆 completed_trips 資料，使用 DELETE 清空")
