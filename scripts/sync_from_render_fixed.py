...
- 內建序列自動校準，無需外部工具。
- 修復: 避免 CASCADE 影響 completed_trips
"""
import os
import sys
import datetime
...
def safe_truncate_and_copy(local_conn, render_conn, table_name):
    """安全地清空本地資料表，並從 Render 完整複製資料，避免影響 completed_trips"""
    ...
    with local_conn.cursor() as local_cur, render_conn.cursor(cursor_factory=DictCursor) as render_cur:
        try:
            ...
            print(f"   - 正在安全清空本地 '{table_name}'...")
            
            # 🔥 修復：統一使用 DELETE 代替 TRUNCATE 以避免外鍵約束問題
            print(f"   - 使用 DELETE 方式清空 {table_name} 表以避免外鍵約束問題...")
...
def main():
    """主函數"""
    print("🚀 開始 Render 資料庫混合模式同步流程 (修復版)")
    print("=" * 60)
    ...
    try:
        ...
        # 步驟 1: 先執行完全覆蓋同步其他表（確保外鍵依賴）
        for table in FULL_SYNC_TABLES:
            safe_truncate_and_copy(local_conn, render_conn, table)

        # 步驟 2: 再執行增量同步 completed_trips（保護歷史資料）
        ...
        print("\n🎉 同步成功完成！")
        print("   - `completed_trips` 已基於日期增量更新，本地歷史資料得到保護。")
