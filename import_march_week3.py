...
def sync():
    print("🚀 連線至遠端資料庫, 準備匯入 3月第3週 資料...")
    engine = create_engine(REMOTE_DB_URL)
    Session = sessionmaker(bind=engine)
    session = Session()

    try:
        # 刪除 03-15 到 03-21 避免重複
        session.execute(text("DELETE FROM completed_trips WHERE date >= '2026-03-15' AND date <= '2026-03-21'"))
        
        with open('march_week3.txt', 'r', encoding='utf-8') as f:
