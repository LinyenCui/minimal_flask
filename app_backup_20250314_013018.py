...
def handle_text_message(event):
    ...
    # 查詢班次（支持指定日期）
        ...
        reply_text = handle_query_trips(message_text)
    
    # 查詢固定班次（支持指定日期）
    elif message_text.startswith("查詢固定班次"):
        reply_text = handle_query_fixed_trips(message_text)
    ...
    try:
        ...
        for trip in completed_trips:
            ...
            try:
                ...
                trip_info = db.session.execute(sql_text(query), {"trip_id": trip_id}).fetchone()
                ...
                print(f"更新班次 #{trip_id} 失敗: {str(e)}")
        
        # 提交事務
        db.session.commit()
        
        return f"✅ 成功更新 {updated_count} 筆已完成班次。"
...
def generate_weekly_report(category=None):
    try:
        ...
        completed_trips = db.session.execute(sql_text(query), query_params).fetchall()
        ...
        # 添加標題
        worksheet.cell(row=1, column=1).value = f"診所班次請款單 ({last_sunday.strftime('%Y/%m/%d')} - {last_saturday.strftime('%Y/%m/%d')})"
...
def handle_generate_weekly_report(text):
    ...
    CATEGORY_FOLDER_MAPPING = {
        "診所": "1Wwp1xIxnn9m9qlvX_BwpE30K0AgLVdYe",  # 添加這一行，替換為實際的診所文件夾 ID
        "東洋": "1dctU8QPRWNPn57LxpcYTeKKcsGn_dLOU"   # 添加這一行，替換為實際的東洋文件夾 ID
    }
    
    # 如果提供了類別但沒有文件夾 ID，嘗試從映射中獲取
    if category and not folder_id and category in CATEGORY_FOLDER_MAPPING:
        folder_id = CATEGORY_FOLDER_MAPPING[category]
    
    # 生成報表
    message, filename = generate_weekly_report(category)
    
    # 如果報表生成成功且有文件夾 ID，則上傳到 Google Drive
    if filename and folder_id:
        upload_message = upload_to_google_drive(filename, folder_id)
        message += f"\n{upload_message}"
    elif filename:
        # 如果沒有指定文件夾 ID，仍然嘗試上傳到 Google Drive 根目錄
        upload_message = upload_to_google_drive(filename)
        message += f"\n{upload_message}"
...
def upload_to_google_drive(file_path, folder_id=None):
    try:
        ...
        return f"文件已上傳到 Google Drive，文件 ID: {file.get('id')}"
    ...
    try:
        ...
        def generate_unique_code(trip_id, date_obj, fixed_trip_id=None):
            ...
            if fixed_trip_id:
                ...
                # 臨時班次：T_班次ID
                return f"T_{trip_id}"
                # 注意：即使不在返回值中使用 week_number，它仍然被定義並在後續的 SQL 更新中使用
        ...
        for trip in trips:
            ...
            # 生成唯一識別碼
            unique_code = generate_unique_code(trip_id, date_obj, fixed_trip_id)
        ...
        completed_trips = db.session.execute(sql_text(completed_query)).fetchall()
        ...
        return f"✅ 成功初始化 {len(trips)} 筆班次和 {len(completed_trips)} 筆已完成班次的唯一識別碼。"
