...
def get_trip_details_flex(trip_id, trip_data):
    ...
    flex_message["body"]["contents"].append({
        ...
        "contents": [
            ...
            },
            {
                ...
                "text": current_status,
                "size": "sm",
                "color": status_color.get(current_status, "#111111"),
                "weight": "bold",
                "flex": 5
            }
        ],
    ...
    flex_message["body"]["contents"].append({
        ...
        "contents": [
            ...
            {
                "type": "text",
                "text": trip_data.get('category') or '未分類',
                "size": "sm",
                "color": "#111111",
                "flex": 5
            }
    ...
    # 添加狀態按鈕
    button_order = ["取消", "衝突", "請假"]
    status_buttons = []
    
    # 為每個狀態創建按鈕，除了當前狀態
    for status in button_order:
        if status != current_status:
            status_buttons.append({
                ...
                "action": {
                    "type": "postback",
                    "label": status,
                    "data": f"action=update_status&trip_id={trip_id}&status={status}",
                    "displayText": f"修改狀態 {trip_id} {status}"
                },
                "color": status_color.get(status, "#999999"),
                "style": "primary",
                "height": "sm",
                "margin": "sm"
            })
...
if __name__ == "__main__":
    ...
    test_data = {
        'date': date(2025, 3, 17),
        'time': datetime.strptime('09:50', '%H:%M').time(),
        'start_point': '仁和路',
        'via_point': None,
        'end_point': '診所',
        'status': '準備',
    ...
    import json
    print(json.dumps(get_trip_details_flex('646', test_data), indent=2, ensure_ascii=False)) 