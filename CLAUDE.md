...
>
> - 核心路由規則（以 {today} 為基準）：
>   - 日期 < {today} → 查過去態 `completed_trips`
>   - 日期 = {today} →
>     - 文本中含「已完成」→ 查 `completed_trips`（今天已經完成的班次）
>     - 否則 → 查 `trips`（今天還在生產線上的班次）
>   - 日期 > {today} → 查現在態 `trips`（因為已匯入的未來班次也在生產線上）
>
> - 用詞統一：資料庫狀態「取消」已更名為「註銷」。所有文件、UI與程式碼請使用「註銷」。
