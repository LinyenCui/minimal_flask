    ...
    try:
        ...
        if new_status == "準備":
            ...
            if has_leave_reason:
                ...
                updated_trip = result.fetchone()
                if updated_trip:
                    success_msg = f"✅ 已成功將班次 #{trip_id} 的狀態設為「準備」。"
                    success_msg += f"\n💰 加成清除：{original_extra_fare} → 0 (變動{-original_extra_fare})"
                    return success_msg
                else:
                    return f"清除班次 #{trip_id} 負數加成時出錯。"
