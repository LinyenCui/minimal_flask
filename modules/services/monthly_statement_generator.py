...
def _get_monthly_statistics(start_date: date, end_date: date, category: str) -> Dict[str, Any]:
    ...
    try:
        ...
        driver_stats = {}
        for row in trips:
            driver_name = row[7] or f"司機{row[6]}"
            amount = row[5] or 0
            if driver_name in driver_stats:
                driver_stats[driver_name] += amount
            else:
                driver_stats[driver_name] = amount
        
        # 前3名司機
        drivers_sorted = sorted(driver_stats.items(), key=lambda x: x[1], reverse=True)
        drivers_top3 = drivers_sorted[:3]
        
        # 獲取入金記錄（可選）
        deposits = _get_recent_deposits(start_date, end_date)
...
def _get_recent_deposits(start_date: date, end_date: date) -> List[Dict[str, Any]]:
    """獲取最近入金記錄"""
    try:
        from sqlalchemy import text
...
def _write_left_info(sheet, start_date: date, end_date: date, stats: Dict[str, Any]):
    ...
    try:
        ...
        sheet['A5'] = '帳戶'
        if stats['deposits']:
            first_deposit = stats['deposits'][0]
            bank_name = first_deposit.get('bank_name', '')
            last4 = first_deposit.get('last4', '＊＊＊＊')
        ...
        for row in range(3, 9):
            if row != 7:  # 跳過空行
                ...
                if row == 8:  # 備註
                    value_cell.font = Font(name='Calibri', size=10, color='666666')
...
def _write_right_amounts(sheet, stats: Dict[str, Any], start_date: date):
    ...
    try:
        ...
        sheet['D8'].font = Font(name='Calibri', size=10, color='CCCCCC')
        ...
        for row in range(9, 11):
            ...
            sheet[f'E{row}'].font = Font(name='Calibri', size=11)
    ...
    try:
        ...
        if stats['deposits']:
            ...
            for i, deposit in enumerate(stats['deposits'][:3], 15):
                deposit_text = f'{deposit["date"]}  NT${deposit["amount"]:,}'
                sheet[f'A{i}'] = deposit_text
                sheet[f'A{i}'].font = Font(name='Calibri', size=10)
        ...
        if stats['drivers_top3']:
            ...
            for i, (driver_name, amount) in enumerate(stats['drivers_top3'], 13):
                ...
                sheet[f'D{i}'].font = Font(name='Calibri', size=10)
